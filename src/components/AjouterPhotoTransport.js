...
export default class AjouterPhotoTransport extends React.Component {
  ...
  async uploadImageAsync(uri) {
    ...
    let pathFile = 'Transport'+'/'+this.state.idMission+'/'+uuid+'.jpg';

    var storageRef = firebase.storage().ref();
    var transportRef = storageRef.child(pathFile);

    const snapshot = await transportRef.put(blob);

    const result = transportRef.getDownloadURL().then(function(url){
      return url
    });
  ...
  resizeImage = async (image) => {
      ...
      const uploadUrl = await this.uploadImageAsync(manipResult.uri)
      .then(() => {
