...
export default class PhotoMaconnerie extends React.Component {

  constructor(props){
    super(props);
    this.state = {
      idMission:this.props.route.params.idMission,
      intitule:this.props.route.params.intitule,
  ...
  getImage = () => {
      let storagePath = firebase.storage().ref().child('Marbrerie/'+this.state.idMission);
  ...
  deleteImage = (name) => {
    var imageRef = firebase.storage().ref().child('Marbrerie/'+this.state.idMission+'/'+name);
  ...
  ajouterMission = async ( idMission, intitule, nom, date, lieu, description ) => {
    let typeMission = "Marbrerie";
    await firebase.database().ref('Missions/'+idMission).set({ typeMission,  idMission, intitule, nom, date, lieu, description })
    .then(()=>{
      ...
      <View style={formulaireMissionStyles.containerChoice}>
        <ScrollView style={{ paddingLeft:15, paddingRight:15}}>
            ...
            <TouchableOpacity
              ...
              onPress={() => this.props.navigation.navigate('AjouterPhotoMaconnerie', {idMission: this.state.idMission})}>
            ...
            <TouchableOpacity
              onPress={() => this.ajouterMission( this.state.idMission, this.state.intitule, this.state.nom, this.state.date, this.state.lieu, this.state.description)}
              style={formulaireMissionStyles.selectChoice2}>
              <Icon type='material' name='arrow-forward-ios' color="#0886f8" size={22} style={formulaireMissionStyles.iconForm}/>
