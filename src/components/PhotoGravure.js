...
export default class PhotoGravure extends React.Component {
  ...
  getImage = () => {
      let storagePath = firebase.storage().ref().child('Gravure/'+this.state.idMission);
  ...
  deleteImage = (name) => {
    var imageRef = firebase.storage().ref().child('Gravure/'+this.state.idMission+'/'+name);
  ...
  ajouterMission = async ( idMission, nom, date, lieu, description ) => {
    let typeMission = "Gravure";
    await firebase.database().ref('Missions/'+idMission).set({ typeMission, idMission, nom, date, lieu, description })
    .then(()=>{
      ...
      <View style={formulaireMissionStyles.containerChoice}>
        <ScrollView style={{ paddingLeft:15, paddingRight:15}}>
            ...
            <TouchableOpacity
              ...
              onPress={() => this.props.navigation.navigate('AjouterPhotoGravure', {idMission: this.state.idMission})}>
            ...
            <TouchableOpacity
              onPress={()=> this.ajouterMission( this.state.idMission, this.state.nom, this.state.date, this.state.lieu, this.state.description )} style={formulaireMissionStyles.selectChoice2}>
              <Icon type='material' name='arrow-forward-ios' color="#0886f8" size={22} style={formulaireMissionStyles.iconForm}/>
